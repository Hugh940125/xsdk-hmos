...
void Ble::setExecuteStartScanCallBack(std::function<void()> callback) { this->executeStartScanCallback = callback; };

void Ble::setExecuteStopScanCallback(std::function<void()> callback) { this->executeStopScanCallback = callback; };

void Ble::setExecuteConnectCallback(std::function<void(string)> callback) { this->executeConnectCallback = callback; };

void Ble::setExecuteDisconnectCallback(std::function<void()> callback) { this->executeDisconnectCallback = callback; };

void Ble::setIsReadyConnectCallback(std::function<bool(string)> callback) { this->isReadyConnectCallback = callback; };

void Ble::setExecuteWriteCallback(std::function<void(const char *, uint16)> callback) {
    this->executeWriteCallback = callback;
};

void Ble::setExecuteWriteCharacteristicCallback(std::function<void(uint16, const char *, uint16)> callback) {
    this->executeWriteCharacteristicCallback = callback;
};

void Ble::setExecuteReadCharacteristicCallback(std::function<void(uint16)> callback) {
    this->executeReadCharacteristicCallback = callback;
};

void Ble::setEnableCharacteristicNotifyCallback(std::function<void(uint16)> callback) {
    this->executeEnableCharacteristicNotifyCallback = callback;
};
...
uint16 Ble::getCharacteristicUUIDF005() {
    if (controller == NULL)
        return 0;
    return controller->getCharacteristicUUIDF005();
}

bool Ble::isPairing() {
    if (controller == NULL)
        return false;
    return controller->isParing();
}
...
void Ble::setDiscoverTimeoutSeconds(uint16 seconds) { discoverTimeoutSeconds = seconds; }
...
void Ble::onScanRespond(string address, int32 rssi, const char *data, uint16 length) {
    ...
    if (connectWhenDiscovered) {
        if (isFoundCurrent(address, name, sn)) {
            pSearchTimer->Cancel();
            if (executeStopScanCallback) {
                executeStopScanCallback();
            } else {
                LOGE("fatal error --> executeStopScanCallback not register")
            }
            if (executeConnectCallback) {
                executeConnectCallback(address);
            } else {
                LOGE("fatal error --> executeConnectCallback not register")
            }
        }
        return;
    }
...
void Ble::onAdvertise(string address, int32 rssi, const char *data, uint16 length) {
    ...
    if (typeid(*controller) == typeid(AidexXController) || dynamic_cast<AidexXController *>(controller) != NULL) {
...
void Ble::onAdvertiseWithAndroidRawBytes(string address, int32 rssi, const char *data, uint16 length) {
    ...
    ByteUtils::copy(bytes, mfrData1 + 2, 1);
    status = LittleEndianByteUtils::byteToUnsignedInt(reinterpret_cast<const uint8 *>(bytes));
    ByteUtils::copy(bytes, mfrData1 + 3, 1);
    calTemp = LittleEndianByteUtils::byteToUnsignedInt(reinterpret_cast<const uint8 *>(bytes));
    ...
    if (iter != controllers.end()) {
        ...
        if (mfrLength1 == BROADCAST_LENGTH) {
            if (typeid(*controller) == typeid(AidexXController) ||
                dynamic_cast<AidexXController *>(controller) != NULL) {
                uint32 u32_ChecksumBase = 0;
                uint8 i;
                for (i = 0; i < BROADCAST_LENGTH - 4; i += 4) {
                ...
                if (crc ==
                    LibChecksum_GetChecksum32Bit((const uint8 *)mfrData1, BROADCAST_LENGTH - 4, u32_ChecksumBase)) {
    ...
    info.rssi = rssi;
    info.status = status;
    info.calTemp = calTemp;
    info.params = vector<uint8>(params);

    for (auto &allIter : controllers) {
        if (allIter.second->sn == sn && allIter.second->mac != address) {
            allIter.second->mac = address;
            allIter.second->doregister();
        }
        if (allIter.second->mac == address) {
            BleController *second = allIter.second;
            second->setInfo(info);
        }
    }
...
void Ble::onConnectSuccess() {
    ...
    controller->setRequireKeepConnect(false);
    controller->setAutoDeleteBond(false);
    ...
    if (pairWhenConnected) {
        if (controller->getDevCommType() == DEV_COMM_TYPE_0) {
            ...
            executeReadCharacteristicCallback(controller->getCharacteristicUUIDF005());
        }
    } else {
        executeReadCharacteristicCallback(controller->getCharacteristicUUIDF005());
    }
...
void Ble::onDisconnected() {
    state = IDLE;
    connectWhenDiscovered = false;
    if (pSearchTimer != nullptr) {
        pSearchTimer->Cancel();
    }
    if (pAckTimer != nullptr) {
        pAckTimer->Cancel();
    }
    if (pDisconTimer != nullptr) {
        pDisconTimer->Cancel();
    }
    commandList.clear();
    if (controller != NULL) {
...
void Ble::onReceiveData(const char *data, uint16 length) {
    ...
    if (controller->isBufferEnabled()) {
        ...
        if (controller->isFrameEnabled() && (state != CONNECTED && state != DISCONNECTING))
            return;
...
void Ble::onReceiveData(uint16 uuid, const char *data, uint16 length) {
    if (controller == nullptr)
        return;
    if (uuid == controller->getCharacteristicUUIDF005()) {
        if (data[0] == 1) {
            if (pairWhenConnected) {
                if (controller->getKey() == nullptr) {
                    pairDenied();
                } else {
                    controller->setAutoDeleteBond(true);
                    executeReadCharacteristicCallback(controller->getCharacteristicUUID());
                }
            } else {
                if (controller->isSupportKeepConnect()) {
                    controller->setRequireKeepConnect(true);
                } else {
                    controller->setAutoDeleteBond(true);
                }
                executeReadCharacteristicCallback(controller->getCharacteristicUUID());
            ...
            if (pairWhenConnected) {
                controller->setKey(nullptr);
                executeEnableCharacteristicNotifyCallback(getPrivateCharacteristicUUID());
            } else {
    ...
    if (uuid == controller->getCharacteristicUUID()) {
        ...
        if (length == controller->getKeyLength() + 1) {
            ...
            if (success) {
                if (controller->isRequireKeepConnect()) {
                    controller->sendCommand(OP_KEEP_LONG_CONNECT, 0, 0, true);
                    return;
                } else if (controller->isAutoDeleteBond()) {
                    controller->sendCommand(OP_DELETE_BOND, 0, 0, true);
                    return;
                } else {
                ...
                if (controller->isAutoDeleteBond()) {
                    pairDenied();
                }
            }
        }
    ...
    if (controller->getDevCommType() == DEV_COMM_TYPE_1 && uuid == controller->getPrivateCharacteristicUUID() &&
        controller->isAuthenticated()) {
        if (controller == NULL)
            return;
        if (controller->isBufferEnabled()) {
            if (buffer != nullptr && buffer->push((uint8 *)data, length)) {
...
void Ble::pairDenied() {
    const uint8_t byteArray[] = {0x00, 0x00};
    controller->handleCommand(0xFF, 0xFF, 0xFF, reinterpret_cast<const uint8 *>(byteArray), sizeof(byteArray));
}

void Ble::startScan() {
    if (!isInConnection()) {
        state = SCANNING;
        executeStartScanCallback();
    }
}
...
bool Ble::pair(BleController *controller) {
    ...
    this->controller->setIsParing(true);
    if (!isInConnection()) {
...
bool Ble::send(BleController *controller, uint8 port, uint8 op, uint8 param, const uint8 *data, uint16 length) {
    ...
    if (!isInConnection()) {
        ...
        pairWhenConnected = false;
        this->controller->setIsParing(false);
        connect();
        return true;
        ...
        if (controller->isAuthenticated() && commandList.empty() && !DevComm::getInstance()->isBusy()) {
            DevComm::getInstance()->send(command.port, Global::MODE_ACKNOWLEDGE, command.op, command.param,
                                         &(command.data)[0], command.data.size());
        } else {
            commandList.push_back(command);
            if (controller->getDevCommType() == DEV_COMM_TYPE_1 && controller->isPaired() &&
                !controller->isAuthenticated() && state == CONNECTED) {
                executeReadCharacteristicCallback(controller->getCharacteristicUUID());
            }
        }
        return true;
...
bool Ble::isInConnection() { return state == CONNECTING || state == CONNECTED; }
    ...
    if (isReadyConnectCallback(mac)) {
        ...
        pSearchTimer->AsyncOnce(discoverTimeoutSeconds * 1000, [this]() {
            if (connectWhenDiscovered) {
                ...
                state = SCANNING;
                if (executeStartScanCallback) {
                    executeStartScanCallback();
                } else {
                    LOGE("fatal error --> executeStartScanCallback not register")
                }
            }
        });
...
void Ble::sendPairCommand() {
    ...
    DevComm::getInstance()->send(controller->getCommPort(), Global::MODE_ACKNOWLEDGE, Global::OPERATION_PAIR,
                                 DEVICE_TYPE, &data[0], data.size());
}

void Ble::sendBondCommand() {
    if (controller == NULL || !controller->isPaired())
        return;
    vector<uint8> data = vector<uint8>(controller->accessId);
    uint8 rxRate = controller->getRxRate();
    if (rxRate > 0)
        data.push_back(rxRate);
    DevComm::getInstance()->send(controller->getCommPort(), Global::MODE_ACKNOWLEDGE, Global::OPERATION_BOND,
                                 DEVICE_TYPE, &data[0], data.size());
...
void Ble::handleCommand(uint8 port, uint8 op, uint8 param, const uint8 *data, uint16 length) {
    ...
    if (port == controller->getCommPort()) {
        ...
        if (op == Global::OPERATION_PAIR)
            bleOp = BleOperation::PAIR;
        if (op == Global::OPERATION_UNPAIR)
            bleOp = BleOperation::UNPAIR;
...
void Ble::write(const uint8 *data, uint16 length) {
    ...
    if (controller == NULL || (state != CONNECTED && state != DISCONNECTING))
        return;
    ...
    if (controller->isBufferEnabled()) {
        ...
        while (length >= i + ATT_MTU_SIZE) {
            ...
            executeWriteCallback(buffer, ATT_MTU_SIZE);
            i += ATT_MTU_SIZE - 1;
        }
        ByteUtils::copy(buffer, (const char *)data + i, length - i);
        buffer[length - i] = end;
        executeWriteCallback(buffer, length - i + 1);
    } else {
        executeWriteCallback((const char *)data, length);
    }
...
void Ble::write(uint16 uuid, const uint8 *data, uint16 length) {
    if (controller == NULL || (state != CONNECTED && state != DISCONNECTING))
        return;
    executeWriteCharacteristicCallback(uuid, (const char *)data, length);
}
...
void Ble::onCharacteristicNotifyEnable(uint16 uuid) {
    if (controller == nullptr)
        return;
