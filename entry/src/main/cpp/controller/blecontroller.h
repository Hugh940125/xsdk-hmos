...
public:
    ...
    virtual uint16 getCharacteristicUUIDF005() const = 0;
    ...
    bool isParing() const { return isPairing; }
    bool isAutoDeleteBond() const { return autoDeleteBond; }
    bool isSupportKeepConnect() const { return supportKeepConnect; }
    bool isRequireKeepConnect(){ return requireKeepConnect; }
    ...
    void setIsParing(const bool pairing) { this->isPairing = pairing;}
    void setAutoDeleteBond(const bool isAuto) { this->autoDeleteBond = isAuto;}
    void setSupportKeepConnect(const bool isSupport) { this->supportKeepConnect = isSupport; }
    void setRequireKeepConnect(const bool require){this->requireKeepConnect = require; }
    ...
    //CGM命令
    virtual uint16 pair();
    virtual uint16 unpair();

    virtual uint16 keepConnect();
...
protected:
    ...
    bool autoDeleteBond;
    bool autoSending;
    bool supportKeepConnect;
    bool requireKeepConnect;
    ...
    virtual bool sendCommand(uint8 op, uint8 *data, uint16 length, bool instantly) { return false;};
